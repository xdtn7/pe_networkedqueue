...
Original file is located at
    https://colab.research.google.com/drive/1844i2mSSzxT1VS-htaGzLUUjLOPFg7Mo
...
mean_pkt_size = 100.0
pkt_size = functools.partial(random.expovariate, 1.0 / mean_pkt_size)
arrival_time = functools.partial(random.expovariate, 1.0 / 0.2)                  #change this value         
service_rate1 = random.expovariate(1.0 / (4*8*mean_pkt_size))           #can process 4 packets/s
service_rate2 = random.expovariate(1.0 / (2*8*mean_pkt_size))           #can process 2 packets/s
service_rate3 = random.expovariate(1.0 / (3*8*mean_pkt_size))           #can process 3 packets/s
...
def plot(x, y, mode = ""):
    ...
    if (mode == "total_occupacy"):
        ...
        fig.suptitle('Average occupacy in the system at the specific time from 0 -> 100ms', fontdict=font)
        ...
        fig, g = plt.subplots(2, constrained_layout=True)
        fig.suptitle("Average waiting time of 1 packet in the system", fontdict=font)
        
        g[0].set_title('Average waiting time', fontdict=font)
...
def main():
    ...
    total_waiting_time = []
    n_total_waiting_time = 1000
    n_total_waiting_time_list = []
    ...
    for i in range(m_replication):
        ...
        total_waiting_time.append(ps[0].waits)

    for i in range(m_replication):
        if (len(total_waiting_time[i]) < n_total_waiting_time):
            n_total_waiting_time = len(total_waiting_time[i])

    n_total_waiting_time_list = list(range(1, n_total_waiting_time + 1))

    for i in range(m_replication):
        while (len(total_waiting_time[i]) < n_total_waiting_time):
            total_waiting_time[i].pop()

    processed_total_waiting_time = deleteInitialData(total_waiting_time, m_replication, n_total_waiting_time)
    ...
    print("Variance of average waiting time: " + str(variance(processed_total_waiting_time[0])))
